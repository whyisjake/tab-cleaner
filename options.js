...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document
    ...
    .addEventListener('click', refreshTabsList);
...
function loadSettings() {
  chrome.storage.sync.get(
    ...
    function (items) {
      ...
      document.getElementById('inactiveTime').value =
...
function saveSettings() {
  const inactiveTimeHours = parseFloat(
    document.getElementById('inactiveTime').value
  );
...
async function refreshTabsList() {
  ...
  container.innerHTML =
    '<div style="padding: 20px; text-align: center; color: #666;">Loading tabs...</div>';
  ...
  try {
    ...
    if (response && response.tabsData) {
      console.log(
        'Received tabs data from background script:',
        response.tabsData.length,
        'tabs'
      );
      ...
      console.error(
        'Failed to get tabs data from background script:',
        response
      );
      container.innerHTML =
        '<div style="padding: 20px; text-align: center; color: #666;">Error loading tabs data. Try refreshing.</div>';
    }
  } catch (error) {
    console.error('Error refreshing tabs list:', error);
    container.innerHTML =
      '<div style="padding: 20px; text-align: center; color: #666;">Error: ' +
      error.message +
      '</div>';
  }
}
...
function displayTabs(tabs) {
  ...
  if (tabs.length === 0) {
    container.innerHTML =
      '<div style="padding: 20px; text-align: center; color: #666;">No tabs found</div>';
    return;
  }
...
async function incrementTabsRemovedCount(count = 1) {
  try {
    ...
    console.log(
      `Updated tabs removed count: ${newCount} (added ${count} from manual close)`
    );
  } catch (error) {
