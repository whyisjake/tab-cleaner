...
async function loadTabStatistics() {
  try {
    ...
    const currentWindowPinned = currentWindow.tabs
      ? currentWindow.tabs.filter(tab => tab.pinned).length
      : 0;
    ...
    document.getElementById('currentWindowTabs').textContent =
      currentWindowTabs;
    document.getElementById('currentWindowPinned').textContent =
      currentWindowPinned;
...
async function loadPauseState() {
  try {
    ...
    const isPaused = result.cleaningPaused || false;
...
async function togglePause() {
  try {
    const result = await chrome.storage.local.get(['cleaningPaused']);
    const currentlyPaused = result.cleaningPaused || false;
    const newPauseState = !currentlyPaused;

    await chrome.storage.local.set({ cleaningPaused: newPauseState });
    ...
    updatePauseUI(newPauseState);

    console.log(`Cleaning ${newPauseState ? 'paused' : 'resumed'}`);
  } catch (error) {
    console.error('Error toggling pause state:', error);
  }
}

// Update pause UI based on state
function updatePauseUI(isPaused) {
  const pauseBtn = document.getElementById('pauseBtn');
  const pauseStatus = document.getElementById('pauseStatus');

  if (isPaused) {
    pauseBtn.textContent = '▶️ Resume Cleaning';
    pauseBtn.classList.add('paused');
    pauseStatus.textContent = 'Cleaning is paused';
  } else {
    pauseBtn.textContent = '⏸️ Pause Cleaning';
...
function displayRecentlyClosedTabs(tabs) {
  ...
  recentTabs.forEach(tab => {
    ...
    const fallbackFavicon =
      'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><rect width="16" height="16" fill="%23ccc"/></svg>';
